...
public class GD_QuanLyKhuyenMai extends JPanel implements ActionListener {
	...
	private void chucNangTimKiem() throws SQLException {
		...
		java.sql.Date sqlNgayKetThuc = new java.sql.Date(ngayBatDau.getTime());
		list = daoKM.TimKiem(txtMaTimKiem.getText().toString(), sqlNgayBatDau, sqlNgayKetThuc);
		int i = 1;
		modelTable.setRowCount(0);
		for (KhuyenMai km : list) {
			String[] row = { i++ + "", km.getMaKhuyenMai(), km.getTenKhuyenMai(), String.valueOf(km.getPhanTram()),
	...
	public boolean checkInput() {
		boolean kq = true;
		int timeStart = Integer.parseInt(cbThoiDiemBatDau.getSelectedItem().toString());
		int timeEnd = Integer.parseInt(cbThoiDiemKetThuc.getSelectedItem().toString());
		if (txtNgayBatDau.getDate().after(txtNgayKetThuc.getDate())) {
			JOptionPane.showMessageDialog(null, "Ngày bắt đầu phải trước ngày kết thúc", "Thông báo nhập liệu",
					JOptionPane.WARNING_MESSAGE);
			return false;
		} else if ((timeStart >= timeEnd) && (txtNgayBatDau.getDate().compareTo((txtNgayKetThuc.getDate())) == 0)) {
			JOptionPane.showMessageDialog(null, "Thời gian bắt đầu phải trước thời gian kết thúc",
					"Thông báo nhập liệu", JOptionPane.WARNING_MESSAGE);
			return false;
		}
