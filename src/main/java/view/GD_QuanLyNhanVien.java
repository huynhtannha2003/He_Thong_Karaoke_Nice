...
public class GD_QuanLyNhanVien extends JPanel implements ActionListener {
	...
	public GD_QuanLyNhanVien() {
		...
		txtMaNhanVien.setText(daoNV.getMaNhanVienGenerate());
		loadData();
	...
	public boolean checkInput() {
		...
		if (txtSDT.getText().matches("^0\\d{9}$") == false) {
			...
			return false;
		} else if (txtEmail.getText().matches("^(.+)@(\\S+)$") == false) {
			JOptionPane.showMessageDialog(null, "Email không hợp lệ", "Thông báo nhập liệu",
					JOptionPane.WARNING_MESSAGE);
			return false;
		}
		return true;
